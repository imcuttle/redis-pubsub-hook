...
class RedisPubSub {
  ...
  async onRequest(channel, data) {
    if (channel === this.requestChannel) {
      const { type, args, uid, from } = JSON.parse(data);
      if (this.uid === from) {
        return;
      }

      const output = await this._runHook('remote', type, args);
      this.pub.publish(this.responseChannel, JSON.stringify({ type, res: output, uid, from: this.uid }));
    }
  }
  ...
  async request(type, ...args) {
    ...
    const onRequest = async (channel, data) => {
      if (channel === this.responseChannel) {
        ...
        const { res, uid: _uid, type: _type } = JSON.parse(data);
        if (_uid !== uid) {
          return;
        }
    ...
    return new Promise(solve => {
      ...
      this.pub.publish(this.requestChannel, JSON.stringify({ type, args, from: this.uid, uid }));
      timer = setTimeout(() => {
        resolve(resList);
      }, this.timeout);
    }).finally(() => {
